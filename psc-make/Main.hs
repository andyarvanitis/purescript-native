...
compile (PSCMakeOptions input outputDir opts usePrefix) = do
  ...
  where
    prefix = if usePrefix
               then ["Generated by pgc-make version " ++ showVersion Paths.version]
               else []
...
main = execParser opts >>= compile
  ...
  headerInfo  = header   "pgc-make - Compiles PureGo to Go"
  footerInfo  = footer $ "pgc-make " ++ showVersion Paths.version
