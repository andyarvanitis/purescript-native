# Xcode support script. Intended to be used with an "External Build System" target.
#
#!/bin/sh

if [[ "$ACTION" == "clean" ]] ; then
  echo "Cleaning all PureScript products..."
  make clean
  ...
  make $OBJECT_TYPE \
       PCCFLAGS=--brief-errors \
       CXXFLAGS="$ARCH_OPTS $FLTO -isysroot $SDKROOT" \
       LDFLAGS="-r -nostdlib $ARCH_OPTS" \
       BIN=purescript.o \
       -j`sysctl -n hw.ncpu`
fi
